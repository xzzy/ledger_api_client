...
class SystemGroup(models.Model):
    ...
    def save(self, *args, **kwargs):
        cache.delete(
            "managed_models.SystemGroup.objects.filter(id=" + str(self.id) + ")"
        )
        cache.delete(
            "managed_models.SystemGroup.get_system_group_member_ids:" + str(self.id)
        )
        cache.delete(
            "managed_models.SystemGroup.objects.filter(name=" + str(self.name) + ")"
        )
        cache.delete(
            "managed_models.SystemGroup.get_system_group_member_ids_active_users:"
            + str(self.id)
        )
...
class SystemUser(models.Model):
    ...
    ledger_id = models.OneToOneField(
        EmailUser,
    ...
    legal_first_name = models.CharField(
        max_length=128, null=True, blank=True, verbose_name="Legal Given name(s)"
    )
    legal_last_name = models.CharField(
        max_length=128, null=True, blank=True, verbose_name="Legal Last name"
    )
    ...
    legal_dob = models.DateField(
        auto_now=False,
        auto_now_add=False,
        null=True,
        blank=True,
        verbose_name="Legal date of birth",
        help_text="",
    )
...
class SystemUserAddress(models.Model):
    ...
    address_type = models.CharField(
        choices=ADDRESS_TYPE, max_length=50, blank=True, null=True
    )
    ...
    def __str__(self):
        return "{} {} {} {} {}".format(
            self.line1, self.locality, self.state, self.country, self.postcode
        )
